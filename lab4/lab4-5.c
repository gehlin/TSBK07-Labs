...
void init(void)
{
	...
	programTerrain = loadShaders("lab4-5terrain.vert", "lab4-5terrain.frag");
	...
	glUniform1i(glGetUniformLocation(programTerrain, "tex1"), 0); // Texture unit 0
	glUniform1i(glGetUniformLocation(programTerrain, "tex2"), 1);
...
void display(void)
{
	...
	glUniform1f(glGetUniformLocation(programTerrain, "t"), t*0.001);
	...
	float tx = 10*(sin(0.0001*t)+1)/2;
	float tz = 10*(cos(0.0001*t)+1)/2;
	mat4 trans = T(tx,getHeight(tx,tz,tm->vertexArray, &ttex),tz);
